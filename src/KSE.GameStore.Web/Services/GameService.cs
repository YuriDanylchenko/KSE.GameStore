...
public class GameService : IGameService
{
    ...
    public async Task<GameDTO> CreateGameAsync(CreateGameRequest createGameRequest) 
    {
        ...
        {
            var regionEntities = await _regionRepository.ListAsync(r => createGameRequest.RegionPermissionIds.Contains(r.Id));
    ...
    public async Task<GameDTO> UpdateGameAsync(UpdateGameRequest updateGameRequest)
    {
        ...
        if (updateGameRequest.Title != gameEntity.Title && (await _gameRepository.ExistsAsync(g => g.Title == updateGameRequest.Title)))
        ...
        {
            var regionEntities = await _regionRepository.ListAsync(r => updateGameRequest.RegionPermissionIds.Contains(r.Id));
            gameEntity.RegionPermissions = regionEntities.ToList();
        }
